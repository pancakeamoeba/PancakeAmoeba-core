...
import "../interfaces/IMainMasterChef.sol";
...
contract StrategyHelper is IStrategyHelper {
    ...
    IMainMasterChef private constant master = IMainMasterChef(0x73feaa1eE314F8c655E354234017bE2193C9E24E);
    ...
    function profitOf(IAMVMinterV2 minter, address flip, uint amount) override external view returns (uint _usd, uint _amv, uint _bnb) {
        ...
        if (address(minter) == address(0)) {
            ...
            _amv = minter.amountAMVToMint(bnbAmount);
        }
        _bnb = 0;
    }
    ...
    function apy(IAMVMinterV2, uint pid) override view public returns(uint _usd, uint _amv, uint _bnb) {
